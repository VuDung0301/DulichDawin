...
export default function FlightsScreen() {
  ...
  useEffect(() => {
    console.log('🚀 FlightsScreen mounted, starting initial fetch...');
    fetchFlights(true);
  }, []);
  ...
  useEffect(() => {
    if (!loading) {
      console.log('🔄 Flight type changed to:', flightType);
      fetchFlights(true);
    }
  }, [flightType]);
  ...
  const fetchFlights = async (reset = true) => {
    console.log('🚀 fetchFlights called with reset:', reset, 'loading:', loading, 'isLoadingMore:', isLoadingMore);
    
    // Chỉ tránh gọi API khi đang load more, không block lần đầu mount
    if (!reset && (loading || isLoadingMore)) {
      console.log('⏸️ Đang loading more, bỏ qua request');
      return;
    }

    // Nếu đang loading mà không phải reset (tức là load more), thì bỏ qua
    ...
    try {
      ...
      console.log('API response đầy đủ:', JSON.stringify(response).substring(0, 500) + '...');
      ...
      if (response.success) {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          console.log('Tìm thấy dữ liệu ở response.data:', flightData.length, 'chuyến bay');
        } 
        // Fallback: Trường hợp dữ liệu nằm trong response.data.data  
        ...
        // Fallback: response là một mảng dữ liệu trực tiếp
        ...
        if (flightData.length > 0) {
          ...
          for (const item of flightData) {
            try {
              ...
              console.error('Lỗi khi chuẩn hóa dữ liệu chuyến bay:', error);
            }
          }
          ...
          console.log('Đã tải và chuẩn hóa được', normalizedFlights.length, 'chuyến bay');
        } else {
  ...
  const normalizeFlightData = (data: any): FlightResponse => {
    try {
      console.log('==== Xử lý dữ liệu chuyến bay ====');
      ...
      // Kiểm tra dữ liệu từ backend Flight model (schema từ MongoDB)
      if (data._id || data.flightNumber || data.airline || data.departureCity || data.arrivalCity) {
        console.log('✅ Đây là dữ liệu từ backend Flight model');
        ...
        // Tạo mã IATA cho sân bay từ tên thành phố
        ...
        const normalizedData = {
          ...
          flight_status: (data.status?.toLowerCase().replace('đúng giờ', 'scheduled').replace('trễ', 'delayed')) || 'scheduled',
          departure: {
        ...
        console.log('✅ Đã chuẩn hóa thành công:', normalizedData.flight.iata);
        return normalizedData;
      }
      
      // Fallback: tạo dữ liệu mẫu
      console.log('⚠️ Không nhận dạng được format, tạo dữ liệu mẫu');
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={{ color: colors.text, marginTop: 10 }}>Đang tải dữ liệu...</Text>
        </View>
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptyText, { color: colors.text }]}>
            {flights.length === 0 
              ? "Không có dữ liệu chuyến bay. Vui lòng kiểm tra kết nối mạng." 
              : "Không tìm thấy chuyến bay nào phù hợp với tìm kiếm"
            }
          </Text>
          <Text style={{ color: colors.tabIconDefault, marginTop: 8, fontSize: 12 }}>
            Tổng số chuyến bay: {flights.length} | Đã lọc: {filteredFlights.length}
          </Text>
          {__DEV__ && (
            <TouchableOpacity 
              onPress={() => {
            ...
            >
              <Text style={{ color: 'white' }}>Debug: Thử tải lại</Text>
            </TouchableOpacity>
          )}
        </View>
      ) : (
